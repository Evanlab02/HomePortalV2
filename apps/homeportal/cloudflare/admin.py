...
class CloudflareZoneAdmin(BaseAdminMixin):
    ...
    def bulk_pull_zones(self, request: HttpRequest, queryset: QuerySet[CloudflareZone]) -> None:
        ...
        try:
            user = request.user
            has_global_perm = user.has_perm(ZONE_PULL_ALL_FQ) or user.has_perm(ZONE_PULL_ANY_FQ)

            records: list[CloudflareZone] = []
            skipped = 0

            for record in queryset:
                if has_global_perm or user.has_perm(ZONE_PULL, record):
    ...
    def has_pull_zone_permission(self, request: HttpRequest, object_id: str | int) -> bool:
        ...
        return (
            user.has_perm(ZONE_PULL_ALL_FQ)
            or user.has_perm(ZONE_PULL_ANY_FQ)
            or user.has_perm(ZONE_PULL, obj)
        )
    ...
    def has_pull_dns_records_permission(self, request: HttpRequest, object_id: str | int) -> bool:
        ...
        return (
            ...
            or user.has_perm(ZONE_PULL_ANY_DNS_FQ)
            or user.has_perm(ZONE_PULL, obj)
            or user.has_perm(ZONE_PULL_DNS, obj)
        )
